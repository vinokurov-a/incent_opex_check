...
Ноутбук запускается ежедневно через Airflow DAG и автоматически определяет, какие проверки нужно выполнить в текущий день на основе настроек из Google Sheets.
...
5. Выводит итоговый отчёт со статусами

## Конфигурация

### Маппинг проверок на ноутбуки

В ноутбуке определён словарь `NOTEBOOK_MAP`:
...
3. Добавьте маппинг в `NOTEBOOK_MAP`
...
- Модуль `env` — для работы с Google Sheets и Redshift
